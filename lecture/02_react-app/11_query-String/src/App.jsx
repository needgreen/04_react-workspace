...
function App() {
  return (
    <>
      <nav>
        <Link to="/">Home</Link> |<Link to="/info">Info 1</Link> |
        <Link to="/info?name=김코딩">Info 2</Link> |
        <Link to="/info?name=박토토&age=22">Info 3</Link>
      </nav>
      <main>
        <Routes>
...
/**
 * 동적 세그먼트와 쿼리 스트링의 차이
 * 1. 동적 세그먼트 - URL에 전달 값을 전달 : /info/김코드/22
 *  1) URL의 특정 위치에 값을 직접 매핑해서 파라미터로 전달
 *  2) 주로 리소스 고유 식별자를 전달할 때 사용
 *  3) 예: 상품 상세페이지, 게시글 상세페이지 등 식별이 중요한 경우 사용
 *  4) 라우트 정의하는 곳에 :id, :userId 처럼 세그먼트를 예약어로 처리함
 *  5) 컴포넌트 측에서 useParams() 훅을 통해 세그먼트 값을 가져올 수 있음 - useParams()는 객체 형태로 반환
 *
 * 2. 쿼리 스트링 - /info?name=김코드&age=22
 *  1) URL 끝의 ? 뒷부분에 여러 정보를 key=value 쌍으로 전달
 *  2)주로 필터링, 검색어, 정렬, 분기 처리 등 페이지 표시/동장 옵션을 넘길때 사용
